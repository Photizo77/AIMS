...
export function AttendanceSummary() {
  const maxTrend = Math.max(...MOCK_SUMMARY.trend.map((t) => t.present));

  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        <div className="bg-white rounded-xl border border-slate-200 border-t-4 border-t-aims-green p-4 shadow-sm text-center"><p className="text-2xl font-extrabold text-slate-900">{MOCK_SUMMARY.present}</p><p className="text-[10px] font-bold text-slate-500 uppercase mt-1">Present Today</p></div>
        <div className="bg-white rounded-xl border border-slate-200 border-t-4 border-t-aims-navy p-4 shadow-sm text-center"><p className="text-2xl font-extrabold text-slate-900">{MOCK_SUMMARY.remote}</p><p className="text-[10px] font-bold text-slate-500 uppercase mt-1">Remote</p></div>
        <div className="bg-white rounded-xl border border-slate-200 border-t-4 border-t-aims-orange p-4 shadow-sm text-center"><p className="text-2xl font-extrabold text-aims-orange mt-0">{MOCK_SUMMARY.late}</p><p className="text-[10px] font-bold text-slate-500 uppercase mt-1">Late Arrivals</p></div>
        <div className="bg-white rounded-xl border border-slate-200 border-t-4 border-t-aims-mint p-4 shadow-sm text-center"><p className="text-2xl font-extrabold text-slate-900">{MOCK_SUMMARY.leave}</p><p className="text-[10px] font-bold text-slate-500 uppercase mt-1">On Leave</p></div>
        <div className="bg-white rounded-xl border border-slate-200 border-t-4 border-t-red-500 p-4 shadow-sm text-center"><p className="text-2xl font-extrabold text-red-500">{MOCK_SUMMARY.absent}</p><p className="text-[10px] font-bold text-slate-500 uppercase mt-1">Absent</p></div>
        <div className="bg-white rounded-xl border border-slate-200 border-t-4 border-t-aims-orange p-4 shadow-sm text-center"><p className="text-2xl font-extrabold text-aims-orange">{MOCK_SUMMARY.geofenceViolations}</p><p className="text-[10px] font-bold text-slate-500 uppercase mt-1">Geofence Failures</p></div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm">
          ...
          <div className="space-y-3">
            {MOCK_SUMMARY.deptPunctuality.map((d) => (
              <div key={d.dept}>
