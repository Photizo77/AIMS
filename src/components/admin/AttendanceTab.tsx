...
const STATUS_STYLES: Record<AttendanceStatus, string> = { present: 'bg-green-100 text-green-700', late: 'bg-yellow-100 text-yellow-700', absent: 'bg-red-100 text-red-700', leave: 'bg-blue-100 text-blue-700', remote: 'bg-purple-100 text-purple-700' };
...
export function AttendanceTab() {
  ...
  const handleSelectType = (type: 'onsite' | 'remote') => {
    ...
    if (type === 'remote') { setGpsCoords('Remote IP: 197.239.xx.xx (Kampala)'); setStep('confirm'); }
    else { setStep('fetching'); setTimeout(() => { setGpsCoords('2.7750° N, 32.2986° E (Gulu Main Campus)'); setStep('confirm'); }, 1500); }
  };

  const handleFinalCheckIn = () => { setShowCheckIn(false); setStep('choose'); setLocationType(null); showToast({ title: 'Attendance Logged', message: `Checked in as ${locationType}.`, type: 'success' }); };
  const resetModal = () => { setShowCheckIn(false); setStep('choose'); setLocationType(null); };

  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div><h2 className="text-lg font-semibold text-slate-900">Attendance & Timesheets</h2><p className="text-sm text-slate-500">Monitor daily presence and weekly hours worked</p></div>
        <button onClick={() => setShowCheckIn(true)} className="px-4 py-2 bg-aims-green text-white rounded-lg text-sm font-bold hover:opacity-90 flex items-center gap-2"><span className="material-symbols-outlined text-[18px]">fingerprint</span>Check In</button>
      </div>
        ...
        <div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-4 mb-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="px-3 py-2 border border-slate-200 rounded-lg text-sm"><option value="all">All Statuses</option><option value="present">Present</option><option value="late">Late</option><option value="remote">Remote</option><option value="absent">Absent</option><option value="leave">Leave</option></select>
              <select value={filterDept} onChange={(e) => setFilterDept(e.target.value)} className="px-3 py-2 border border-slate-200 rounded-lg text-sm"><option value="all">All Departments</option><option>Executive</option><option>Finance</option><option>Grants</option><option>Administration</option><option>Innovation</option></select>
              <select value={filterCheckType} onChange={(e) => setFilterCheckType(e.target.value)} className="px-3 py-2 border border-slate-200 rounded-lg text-sm"><option value="all">All Records</option><option value="checked_in">Checked In</option><option value="not_checked_in">Not Checked In</option></select>
      ...
      {activeView === 'timesheets' && (
        <div className="bg-white rounded-xl border border-slate-200 overflow-x-auto">
          <div className="p-4 border-b border-slate-200 flex justify-between items-center"><h3 className="font-bold text-slate-900">Weekly Timesheet (Aug 1 - Aug 5)</h3><button className="text-xs font-bold text-aims-navy hover:underline flex items-center gap-1"><span className="material-symbols-outlined text-[16px]">download</span>Export PDF</button></div>
          <table className="w-full text-sm">
            <thead><tr className="bg-slate-50 border-b border-slate-200 text-slate-600">
              <th className="text-left px-4 py-3 font-semibold">Employee</th><th className="text-center px-2 py-3 font-semibold">Mon</th><th className="text-center px-2 py-3 font-semibold">Tue</th><th className="text-center px-2 py-3 font-semibold">Wed</th><th className="text-center px-2 py-3 font-semibold">Thu</th><th className="text-center px-2 py-3 font-semibold">Fri</th><th className="text-center px-4 py-3 font-bold text-slate-900">Total</th><th className="text-center px-4 py-3 font-semibold">Status</th>
            </tr></thead>
            <tbody>
              {MOCK_TIMESHEETS.map((t, i) => (
                <tr key={i} className="border-b border-slate-50 hover:bg-slate-50">
                  <td className="px-4 py-3 font-bold text-slate-900">{t.name}</td>
                  <td className="text-center px-2 py-3 text-slate-600">{t.mon}h</td><td className="text-center px-2 py-3 text-slate-600">{t.tue}h</td><td className="text-center px-2 py-3 text-slate-600">{t.wed}h</td><td className="text-center px-2 py-3 text-slate-600">{t.thu}h</td><td className="text-center px-2 py-3 text-slate-600">{t.fri}h</td>
                  <td className="text-center px-4 py-3 font-extrabold text-aims-navy">{t.total}h</td>
                  <td className="text-center px-4 py-3"><span className={cn('px-2 py-1 rounded-full text-[10px] font-bold uppercase', t.status === 'Approved' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700')}>{t.status}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
            ...
            {step === 'choose' && (
              <div className="space-y-4">
                ...
                <button onClick={() => handleSelectType('onsite')} className="w-full py-4 rounded-xl border-2 border-slate-200 hover:border-aims-green hover:bg-green-50/30 transition-all flex items-center gap-4 px-4">
                  ...
                  <div className="text-left"><p className="font-bold text-slate-900">Physical / On-Site</p><p className="text-xs text-slate-500">Will capture GPS coordinates</p></div>
                </button>
                <button onClick={() => handleSelectType('remote')} className="w-full py-4 rounded-xl border-2 border-slate-200 hover:border-aims-navy hover:bg-blue-50/30 transition-all flex items-center gap-4 px-4">
                  ...
                  <div className="text-left"><p className="font-bold text-slate-900">Remote / WFH</p><p className="text-xs text-slate-500">Logs IP address instead</p></div>
                </button>
              </div>
            )}
            {step === 'fetching' && (
