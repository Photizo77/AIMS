...
// ============================================================
// AIMS — Full-Page AI Writing Assistant
// ============================================================
...
interface Model { id: ModelId; name: string; badge: string; strength: string; }
    ...
    try {
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          ...
          model: selectedModel,
          systemPrompt: `You are an expert grant writing assistant for Ardhi. ${selectedContext !== 'general' ? `Context: ${selectedContext}.` : ''} Write professionally.`,
        }),
      });
  ...
  return (
    <div className="h-[calc(100vh-100px)] flex flex-col bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      <div className="flex items-center justify-between px-5 py-3 border-b border-slate-200 bg-slate-50 shrink-0">
        ...
        <div className="flex items-center gap-3">
          <select value={selectedContext} onChange={(e) => setSelectedContext(e.target.value)} className="hidden sm:block px-3 py-1.5 border border-slate-200 rounded-lg text-xs font-medium text-slate-700 bg-white max-w-[220px]">
            {GRANT_CONTEXTS.map((ctx) => (<option key={ctx.id} value={ctx.id}>{ctx.label}</option>))}
          </select>
          <div className="relative">
            <button onClick={() => setIsModelMenuOpen(!isModelMenuOpen)} className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-slate-200 hover:bg-white text-xs font-semibold text-slate-700 bg-white">
            ...
            {isModelMenuOpen && (
              <>
                ...
                <div className="absolute right-0 top-full mt-1 w-64 bg-white rounded-lg border border-slate-200 shadow-lg z-20 overflow-hidden">
                  {MODELS.map((model) => (
                    <button key={model.id} onClick={() => { setSelectedModel(model.id); setIsModelMenuOpen(false); }} className={cn('w-full text-left px-3 py-2.5 hover:bg-slate-50 border-b border-slate-100 last:border-0', selectedModel === model.id && 'bg-slate-50')}>
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-5">
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center max-w-lg mx-auto">
            ...
            <p className="text-sm text-slate-500 mb-8">Select a grant context above, then ask me to draft, refine, or brainstorm.</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 w-full">
              {suggestedPrompts.map((prompt) => (
                <button key={prompt} onClick={() => setInput(prompt)} className="text-left px-4 py-3 rounded-lg border border-slate-200 hover:border-aims-green hover:bg-green-50/30 text-xs font-medium text-slate-700 transition-colors">
                  {prompt}
                </button>
              ))}
            </div>
          ...
          <>
            {messages.map((msg, i) => (
              <div key={i} className={cn('flex gap-3', msg.role === 'user' ? 'justify-end' : 'justify-start')}>
                ...
                <div className={cn('max-w-[75%] rounded-2xl px-4 py-3 text-sm', msg.role === 'user' ? 'bg-aims-navy text-white rounded-br-sm' : 'bg-slate-100 text-slate-900 rounded-bl-sm')}>
      ...
      <div className="p-4 border-t border-slate-200 bg-white shrink-0">
        <div className="max-w-3xl mx-auto flex items-end gap-2 bg-slate-50 rounded-xl border border-slate-200 p-2 focus-within:border-aims-green transition-colors">
          <textarea ref={textareaRef} value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={handleKeyDown} placeholder={`Message ${currentModel.name}...`} rows={1} className="flex-1 bg-transparent px-3 py-2 text-sm text-slate-900 placeholder-slate-400 resize-none focus:outline-none max-h-48" />
          <button onClick={handleSend} disabled={!input.trim() || isLoading} className={cn('w-10 h-10 rounded-lg flex items-center justify-center shrink-0 transition-colors', input.trim() && !isLoading ? 'bg-aims-green text-white hover:opacity-90' : 'bg-slate-200 text-slate-400 cursor-not-allowed')}>
