...
export function PayslipsTab() {
  ...
  const requestRevisions = () => {
    setPayslips((prev) => prev.map((p) => (p.status === 'pending' ? { ...p, status: 'rejected', notes: 'Returned for revision — recalculate items' } : p)));
    showToast({ title: 'Revisions Requested', message: 'Payroll batch returned to Company Admin for revision.', type: 'warning' });
  };

  const downloadPayslip = (p: PayslipRecord) => {
    exportRecordSheet(`${p.employeeName} payslip`, 'Payslip Record', [
      ['Employee', p.employeeName], ['Position', p.position], ['Period', p.period],
      ['Base Salary', `UGX ${p.baseSalary.toLocaleString()}`], ['Allowances', `UGX ${p.allowances.toLocaleString()}`],
      ['Deductions', `UGX ${p.deductions.toLocaleString()}`], ['Net Pay', `UGX ${p.netPay.toLocaleString()}`],
      ['Status', p.status], ['Generated By', p.generatedBy],
    ]);
    showToast({ title: 'Payslip Downloaded', message: `${p.employeeName} — ${p.period} record sheet.`, type: 'success' });
  };
      ...
      {isED && pendingCount > 0 && (
        <div className="bg-white rounded-xl border border-aims-navy/30 border-l-4 border-l-aims-navy p-5 mb-4 shadow-sm">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            <button onClick={requestRevisions} className="px-4 py-2 bg-aims-orange text-white text-xs font-bold rounded-lg hover:bg-aims-orange/90">◄ Request Revisions</button>
          </div>
        </div>
      )}
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden" onClick={function(e) { e.stopPropagation(); }}>
            ...
            <div className="p-6 space-y-3">
              ...
              <div className="flex justify-end gap-2 pt-3 border-t border-slate-100">
                <button onClick={function() { downloadPayslip(selectedPayslip); }} className="px-4 py-2 bg-aims-navy text-white text-xs font-bold rounded-lg">Download Payslip</button>
              </div>
            </div>
          </div>
        </div>
